...
An item that `tasks-axi prune` already retired out of the active backlog is closed against its archive instead: on the markdown backend the close is satisfied only when exactly one `data/done-archive.md` entry names the id, the archived record is read and never rewritten, and completion says the original archived outcome stands rather than claiming a fresh close (any other adapter has no supported archive read and refuses).
Because that record is never rewritten, a close carrying a `--pr` or `--report` the archived entry does not already record refuses and keeps its pending-close record, so the delivery evidence survives for a hand reconcile; only the generated `--note` is reported as not reapplied.
A recorded retention (`mode=retain`) whose row is absent likewise refuses, because an archived Done entry proves someone completed the row, never that the captain answered the question the retention recorded.
...
Lifecycle access to a backlog, its done archive, a task record, or a pending-close record must resolve within its configured data or state root, and a final-component symlink is refused even when its target remains within that root.
