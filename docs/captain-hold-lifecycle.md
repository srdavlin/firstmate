...
If the row has left the active backlog altogether by the time replay runs, the retention refuses and keeps its record and arguments for a hand reconcile: an archived Done entry is evidence that someone completed the row, never the captain's answer, so replay never converts a recorded retention into a close.
