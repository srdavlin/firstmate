...
- `BOOTSTRAP_INFO: finished the interrupted cleanup for <id>; its backlog item was already completed and archived, so its original outcome stands` - the row had been pruned out of the active backlog, replay verified it in this home's archive, and nothing was rewritten there; the line names a generated note that was not reapplied when one was carried.
  No action is owed; the `...against its already-archived backlog item...` wording of the same line means physical cleanup was interrupted, so verify process reaping, the local-copy return, and endpoint closure as with the other interrupted-cleanup lines.
- `BACKLOG_RECONCILE: <id>: recorded backlog close could not be replayed: <reason>` - this session start found a pending-close record carrying a close or retention transition but could not land it.
  ...
  A reason naming the archive means the row is no longer in the active backlog: it is absent from the archive too, matches it ambiguously, or is archived with an outcome that does not record the pull request or report this close carries.
  Repair that by hand - recover or reconcile the row, or record the named artifact in the archive entry - and rerun session start; the archived record is never rewritten for you, and a recorded retention whose row is gone always needs the captain's answer rather than an archived Done entry.
